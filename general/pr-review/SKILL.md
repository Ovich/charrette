...
5. **Blast radius**: what depends on the changed surface: callers of changed
   ...
   that must accompany the change, behavior changes observable by users. Verified by
   searching the repo, not assumed from the diff.
6. **Decision points**: the questions only the reviewing human can answer, each
   ...
   (schema migrations, API contract changes, dropped compatibility).
7. **Findings** (§ Findings): per axis, each with its citation.
8. **Verdict**: one line per axis (worst issue), plus the open decision count.
...
9. **Proposed comment**: the whole review condensed into a PR comment the reviewer
   ...
   **Approve with comments** / **Request changes**), then at most ~150 words: what
   ...
   Recommendation mapping: blocking finding on either axis → Request changes;
   nothing blocking but open decisions or non-blocking findings → Approve with
   comments; clean on both axes and no open decisions → Approve.
...
Merge-dedup, rank by cost, drop anything the project's own linter/CI already flags.
Findings are hypotheses until the citation is read: the citation requirement is
what makes them checkable.

For deeper axes, chain the siblings instead of duplicating them: design cost →
`code-design-review`; React quality → `frontend-review`; bug hunt → the harness's
own review. Link their reports from the analysis document rather than inlining.
