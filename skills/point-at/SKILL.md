...
**A screen is talked about with the screen in front of the person, in the mockup's own names**, whether the agent is asking or answering. A region named in the agent's words ("the door", "shell middle") comes back as "what is that?", and the question is lost with it.
...
6. **When the answer is in, `show --done`**: the person's tab goes back to what they were reading before the first pointer. Between two questions in a row there is no `--done`, it closes the run. Forgotten, the person is left on a mockup with a highlight nobody is talking about.
7. **No mockup draws it**: say so in the question and cite the screen's `file:line`. A layout that exists nowhere is never described in prose for the person to choose in: it goes to the `frontend-design` skill first.
