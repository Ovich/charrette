...
it (the documents, by name, and their groups once the slot has been split into pieces
of work), its derived state, what it depends on. An iteration with no customer-facing
outcome, the foundation, states the operator's check instead: what runs where, and
what a commit does. Stories are not here: they live in the spec of the slot that
makes them true. The unscheduled pool is one line per idea not yet placed, so nothing
is lost and nothing is designed early.
...
product has one (provider, tiering, where the prompts and rules live), and any
technology decision the boards hold that a slot will build on (a rendering engine, a
payments provider), added as a row. The engine's internal design is a slot's board,
not a row. Once code exists the file is a cache of
