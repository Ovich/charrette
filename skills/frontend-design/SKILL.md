...
4. **Render and iterate**: write the file, register and serve it, tell the user the URL, edit the same file as they react. Hand it over as something to change. **A mockup is a working prototype, not a picture**: the behaviour is mocked in the file so the flow can be walked through. Once the visual pass is approved, ask about the flow and mock it. A question about a region of a mockup already rendered is asked through the `point-at` skill. Check every variant and the three viewport presets before calling it done.
