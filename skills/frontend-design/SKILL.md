...
1. **Load or build the design language** (below). If the project's
   ...
   "does this match how you see the product?". No codebase to extract from, or a
   codebase with no language of its own yet (a scaffold, a template, defaults never
   chosen): `references/greenfield.md`, which opens on the few questions to ask first,
   the stack and its UI library above all.
