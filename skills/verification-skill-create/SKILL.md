...
**Two things are produced, and a later agent runs from them alone**: the project's `verify-<app>` skill, which says how to launch, drive, observe and clean up, and the feature map in aiview, which says how a story's consumer reaches the part of the application to verify. Neither is documentation for a person, though a person reads both.

## Before writing

- **Stop if a `verify-<app>` skill exists** in the project's local skill folder. Say so, name `verification-skill-maintain` (`../verification-skill-maintain/SKILL.md` in this collection), and ask whether a second skill under another name is wanted. Never overwrite one.
- **Establish from the repository, never by asking**: the consumer (a person at a screen or a shell, a client, an agent, another service), the interface they use, how the system starts locally and signals ready, how that interface can be driven, how outcomes and side effects can be observed, what state a story needs. The README, the start scripts, an existing harness, the routes, the commands, the contract.
...
- **Stories come from the design**: the spec, the roadmap's slots, the README's promises, the issues, in that order. None: ask the person for the three to five journeys the product exists for. Never invent one.
...
**One `feature-map` document per project, opened in aiview on creation** (`../aiview/SKILL.md` in this collection): kind `feature-map` from the filename `YYYY-MM-DD-<project>.feature-map.md`, tags = project + `verification`, no group, started when the reading began. Tell the person the URL.
...
2. **Navigation**: one mermaid flowchart from the entry point through the features to their sub-features, each edge labelled with the consumer's action that takes it (a click, a command, a call, a message). Drawn to the `write-diagrams` skill (`../write-diagrams/SKILL.md` in this collection), parsed with `aiview mermaid-check` after every edit.
...
4. **Stories**, a table: id, the story in the consumer's words, the features on its path in order, the starting state, the expected outcome, the side effects that matter. The id is the spec's when the spec has one.
...
| Thought | Reality |
|---|---|
| "I'll ask which framework they use" | The repository says. Ask only what it cannot. |
...
| "It fails on the app, I'll patch the app" | A finding, reported apart. The skill is proven on the application as it is. |
