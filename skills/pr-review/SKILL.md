...
A spec reaches the review when the person names it or the PR's group tag finds it
(`aiview list --kind spec --tag <group>`); a named spec makes L5 material whatever
the script says. A PR with no stated intent skips L5 entirely: *"L5: not run, no
stated intent."*
