...
| [execute-slice](skills/execute-slice/SKILL.md) | One slice document is handed to an agent to carry out | *"Do slice 2."* Checks the blockers first and stops if any is unmet, writes the code and the tests the slice owes, in the rhythm the mandate names (at the slice end, commit only at green, or commit per phase), returns the branch and the run as evidence. |
